...
if file.open("state.txt", "r") then
	...
	print("can not read state.txt")
end

if file.open("map.txt", "r") then
	for i=1,20 do
		anim.map[i] = tonumber(string.sub(file.readline(),1,-2) or "1")
	end
	file.close()
else
	print("can not read map.txt")
end
